...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const bgSelect = document.getElementById("bk-background");
  const pkgSelect = document.getElementById("bk-package");
  const bgPreview = document.getElementById("bk-bg-preview");
  const bgPreviewImages = document.getElementById("bk-bg-preview-images");
  const bgPreviewLabel = document.getElementById("bk-bg-preview-label");
  ...
  function updateBgPreview() {
    const photos = CONFIG.BACKGROUND_PHOTOS[bgSelect.value];
    if (!photos) {
      bgPreview.style.display = "none";
      return;
    }
    const list = Array.isArray(photos) ? photos : [photos];
    bgPreviewImages.innerHTML = "";
    list.forEach(src => {
      const img = document.createElement("img");
      img.src = src;
      img.alt = bgSelect.value;
      bgPreviewImages.appendChild(img);
    });
    bgPreviewLabel.textContent = bgSelect.value;
    bgPreview.style.display = "flex";
  }

  catSelect.addEventListener("change", () => { populatePackages(); populateBackgrounds(); updateSummary(); });
